...
export class AuthService {
    ...
    createUser = async()=>{
      ...
      const res = await this.db.users.create({data:{code:pass,role:'admin'}})
      return res
    }

    validateUser = async (pass:string): Promise<{id:number,role:string}|null> => {
        const admin = await this.db.users.findFirst({where:{role:'admin'}});
